...
const paramsSchema = z.object({
  imagem: z.string().min(1, { message: 'Imagem não selecionada' }),
})
...
export default defineEventHandler(async event => {
  ...
  const { imagem: nomeArquivo } = params.data
  ...
  const imagem = await readFile(`${FILES_PATH}user/${basename(nomeArquivo)}`)
